...
% Słowa kluczowe:
\keywords{uczenie maszynowe, ukryte indeksowanie semantyczne, model przestrzeni wektorowej, systemy rekomendacyjne, korpus, metoda gradientowa, stochastyczny gradientowy spadek, SVD, gensim}
...
W terminach \texit{statystycznej teorii uczenia} opracowanej przez Vladimira Vapnika \cite{Vapnik} celem algorytmów uczenia maszynowego jest znalezienie parametru $\theta^{\ast}$ minimalizującego funkcję \textit{oczekiwanego ryzyka} (ang. \textit{expected risk}) $Q$:
...
\While{niespełniony warunek stopu}{
    $(u, i) \gets $ losowy punkt ze zbioru $T$ \;
    \ForEach{$k \in range(0, dim(\mathbf{Q}_{u \ast}))$}{
        $q'_{uk} \gets q_{uk} - \alpha \frac{\partial}{\partial q_{uk}} c(a_{ui}, \mathbf{Q}_{u \ast}, \mathbf{R}_{\ast i})$\;
        $r_{ki} \gets r_{ki} - \alpha \frac{\partial}{\partial r_{ki}} c(a_{ui}, \mathbf{Q}_{u \ast}, \mathbf{R}_{\ast i})$\;
        $q_{uk} \gets q'_{uk}$ \;}}
...
Istotnym problemem pojawiającym podczas procesów uczenia maszynowego jest \textit{przeuczenie} zwane też \texit{nadmiernym dopasowaniem}, kiedy algorytm osiąga dobre rezultaty na danych treningowych, ale jego skuteczność znacząco spada dla nowych przykładów, które nie pojawiły się w procesie uczenia. Strategia osiągania dobrego poziomu generalizacji algorytmu jest znana jako \textit{regularyzacja}. Ideą stojąca za regularyzacją jest fakt, że modele zbyt złożone i elastyczne zbytnio dopasowują się do analizowanych danych. 
...
\begin{algorithm}
\SetAlgorithmName{Algorytm}{}
\KwIn{$\eta$ --- współczynnik wag zerowych,  $\Delta$ --- przetwarzany zbiór dokumentów, $f$ --- metoda obliczania wagi słowa, $V$ --- zbiór walidacyjny}
...
\toprule
Klasyfikator                             & waga = 0.0 & waga = 1.0 & waga = 2.0 & waga = 3.0 & waga = 5.0 \\ \midrule
...
\begin{tabular}{@{}lllll@{}}
\toprule
Model semantyczny & Słownik pojęć    & mała regularyzacja, 400 cech & duża regularyzacja, 400 cech & duża regularyzacja, 200 cech \\ \midrule
cały korpus       & cały korpus      & 0.779                        & 0.796                        & 0.776                        \\
początkowy zbiór  & początkowy zbiór & 0.750                        & 0.808                        & 0.799                        \\
początkowy zbiór  & cały korpus      & 0.720                        & 0.793                        & 0.810                        \\ \bottomrule
\end{tabular}%
}
...
\begin{tabular}{@{}lll@{}}
\toprule
Koncept & Miara istotności & Najważniejsze słowa z określającymi je wagami                    \\ \midrule
0       & 51.032           & 0.453 * 'und' + 0.406 * 'die' + 0.395 * 'der' + 0.157 * 'zu'     \\
        &                  & + 0.143 * 'mit' + 0.138 * 'eine' + 0.136 * 'von'                 \\
        &                  & + 0.130 * 'ist' + 0.125 * 'des' + 0.125 * 'het'                  \\ \midrule
1       & 46.728           & 0.369 * 'het' + 0.326 * 'een' + 0.257 * 'de' + 0.229 * 'en'      \\
        &                  & + 0.190 * 'van' + -0.172 * 'und' + 0.154 * 'dat'                 \\
        &                  & + 0.145 * 'voor' + -0.143 * 'der' + 0.140 * 'patients'           \\ \midrule
2       & 45.496           & -0.252 * 'het' + 0.246 * 'patients' + -0.222 * 'een'             \\
        &                  & + 0.176 * 'icu' + -0.137 * 'en' + -0.125 * 'de' + -0.113 * 'van' \\
        &                  & + 0.109 * 'cells' + -0.105 * 'dat' + -0.099 * 'voor'             \\ \midrule
3       & 34.429           & -0.411 * 'les' + -0.328 * 'la' + -0.282 * 'des' + -0.235 * 'de'  \\
        &                  & + -0.222 * 'une' + -0.212 * 'dans' + -0.206 * 'que'              \\
        &                  & + -0.198 * 'le' + -0.174 * 'est' + -0.167 * 'un'                 \\ \bottomrule
\end{tabular}%
}
\caption{Najważniejsze koncepty w tekstach wielojęzycznych}
\label{multilingual}
\end{table}

Ponieważ w zadaniu skupiono się na budowie korpusu jedynie na podstawie dokumentów anglojęzycznych, należało wyeliminować pozostałe teksty. Posłużono się biblioteką \textit{langdetect}\footnote{https://pypi.python.org/pypi/langdetect}, która pozwala na wykrycie języków występujących w danym tekście oraz przekazuje informację o prawdopodobieństwie ich predykcji.
...
\bibitem[1]{paperity} Oficjalna strona internetowa serwisu Paperity --- \url{http://paperity.org/}, data dostępu: 1.10.2017

\bibitem[2]{funk} Simon Funk, \textit{Netflix Update: Try This at Home},\\ \url{http://sifter.org/\textasciitilde simon/journal/20061211.html}, 2006, data dostępu: 1.10.2017

\bibitem[3]{gensim} Oficjalna strona internetowa biblioteki gensim --- \url{https://radimrehurek.com/gensim/}, data dostępu: 1.10.2017

\bibitem[4]{polettini} Nicola Polettini, \textit{The Vector Space Model in Information Retrieval --- Term Weighting Problem}, Uniwersytet w Trento, 2004

\bibitem[5]{sparck} Karen Sparck Jones, A Statistical Interpretation of Term Specificity And Its Application In Retrieval, \textit{Journal of Documentation}, 28, 11 -- 21, Emerald Group Publishing, 1972

\bibitem[6]{takacs} Gabor Takacs, Istvan Pilaszy, Bottyan Nemeth, Domonkos Tikk, Scalable Collaborative Filtering Approaches for Large Recommender Systems, \textit{Journal of Machine Learning Research}, 10, 623 -- 656, JMLR, Inc, 2009

\bibitem[7]{bottou} Leon Bottou, Stochastic Gradient Descent Tricks, \textit{Neural Networks: Tricks of the Trade}, 421 -- 436, Springer, 2012

\bibitem[8]{bottou2} Leon Bottou, Large-Scale Machine Learning with Stochastic Gradient Descent, \textit{Proceedings of COMPSTAT'2010}, 177 -- 186, Springer, 2010

\bibitem[9]{Vapnik} Vladimir N. Vapnik, An Overview of Statistical Learning Theory, \textit{IEEE Transactions On Neural Networks}, 10(5), 988 -- 999, IEEE Computational Intelligence Society, 1999

\bibitem[10]{gemulla} Rainer Gemulla, Peter J. Haas, Erik Nijkamp, Yannis Sismanis, Large-Scale Matrix Factorization with Distributed Stochastic Gradient Descent, \textit{Proceedings of the 17th ACM SIGKDD international conference on Knowledge discovery and data mining}, 69 -- 77, ACM, 2011

\bibitem[11]{landauer} Thomas K. Landauer, Peter W. Foltz, Darrell Laham, An Introduction to Latent Semantic Analysis, \textit{Discourse Processes}, 25, 259 -- 284, Routledge, 1998

\bibitem[12]{landauer2} Thomas K. Landauer, Susan T. Dumais, A Solution to Plato's Problem: The Latent Semantic Analysis Theory of Acquisition, Induction, and Representation of Knowledge, \textit{Psychological Review}, 1(2), 211 -- 240, American Psychological Association, 1997

\bibitem[13]{leskovec} Jure Leskovec, Anand Rajaraman, Jeffrey D. Ullman, \textit{Mining of Massive Datasets}, Cambridge University Press, 2014

\bibitem[14]{rehurek} Radim Rehurek, \textit{Scalability of Semantic Analysis in Natural Language Processing}, Uniwersytet Masaryka, 2011

\bibitem[15]{suthaharan} Shan Suthaharan, \textit{Machine Learning Models and Algorithms for Big Data Classification}, Springer, 2015

\bibitem[16]{rendle} Steffen Rendle, Lars Schmidt-Thieme, Online-Updating Regularized Kernel Matrix Factorization Models for Large-Scale Recommender Systems, \textit{Proceedings of the 2008 ACM conference on Recommender systems}, 251 -- 258, ACM, 2008

\bibitem[17]{grossman} Giovanni Seni, John Elder, \textit{Ensemble Methods in Data Mining: Improving Accuracy Through Combining Predictions}, Morgan \& Claypool, 2010

\bibitem[18]{chapelle} Olivier Chapelle, Bernhard, Schölkopf, Alexander Zien, \textit{Semi-supervised learning}, MIT Press, 2006

\bibitem[19]{zadrozny} Bianca Zadrozny, Charles Elkan, Transforming Classifier Scores into Accurate Multiclass Probability Estimates, \textit{Proceedings of the eighth ACM SIGKDD international conference on Knowledge discovery and data mining}, 694 -- 699, ACM, 2002
